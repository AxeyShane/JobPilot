"""JobPilot HTML Dashboard Generator.
...
from jobpilot.config import APP_DIR, DB_PATH
...
def generate_dashboard(output_path: str | None = None) -> str:
    ...
    Args:
        output_path: Where to write the HTML file. Defaults to ~/.jobpilot/dashboard.html.
    ...
    site_stats = conn.execute("""
        SELECT site,
               COUNT(*) as total,
               SUM(CASE WHEN fit_score >= 6 THEN 1 ELSE 0 END) as high_fit,
