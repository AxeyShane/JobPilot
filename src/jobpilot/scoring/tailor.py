...
from jobpilot.config import RESUME_PATH, TAILORED_DIR, load_profile
...
def compute_keyword_match_rate(score_reasoning: str | None, resume_text: str) -> float | None:
    """Fraction of the job's scoring-stage keywords that appear in the
    tailored resume text.

    Free signal: the score stage already extracts per-job ATS keywords
    (stored as the first line of score_reasoning, see scorer.py), and this is
    the tailored resume we already produced -- comparing them is a string
    match, no LLM call, near-zero marginal cost. Gives a per-application
    quality number without asking the model to grade its own homework.

    Returns None if there are no keywords to compare against (nothing to
    compute), rather than a misleading 0.0.
    """
    if not score_reasoning:
        return None
    keywords_line = score_reasoning.split("\n", 1)[0]
...
- ALL of these companies MUST appear as separate entries in "experience" -- {companies_str}.
  Every one, every time, with no exceptions, even if a company's work looks irrelevant to
  this job -- reframe its bullets to whatever angle is relevant instead of dropping the
  entry. Only PROJECTS may be dropped entirely for irrelevance; experience entries may not.
- Names of preserved companies stay exactly as written, do not paraphrase or abbreviate them.
- Preserved school: {school}
- Must fit 1 page -- trim bullet count per entry (not entries themselves) if space is tight.

## OUTPUT: Return ONLY valid JSON. No markdown fences. No commentary. No "here is" preamble.

{{"title":"Role Title","summary":"2-3 tailored sentences.","skills":{{"Languages":"...","Frameworks":"...","DevOps & Infra":"...","Databases":"...","Tools":"..."}},"experience":[{{"header":"Title at Company","subtitle":"Month Year - Month Year","bullets":["bullet 1","bullet 2","bullet 3","bullet 4"]}}],"projects":[{{"header":"Project Name - Description","subtitle":"Month Year - Month Year","bullets":["bullet 1","bullet 2"]}}],"education":"{school} | {education_level}"}}"""
...
def tailor_resume(
    ...
    for attempt in range(max_retries + 1):
        ...
        raw = client.chat(messages, max_tokens=2048, temperature=0.15)
...
def run_tailoring(min_score: int = 6, limit: int = 0,
    ...
    Args:
        ...
        urls:            Tailor only these job URLs (fast lane).
    ...
    jobs = get_jobs_by_stage(conn=conn, stage="pending_tailor", min_score=min_score,
                             limit=limit, urls=urls, newest_first=newest_first)
    ...
    def _tailor_one(job: dict) -> dict:
        try:
            ...
            return {
                ...
                "keyword_match_rate": compute_keyword_match_rate(job.get("score_reasoning"), tailored),
            }
    ...
    log.info("Tailoring resumes for %d jobs (score >= %d, %d concurrent)...", len(jobs), min_score, workers)
    t0 = time.time()
    completed = 0
    results: list[dict] = []
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        ...
        for future in as_completed(futures):
            ...
            if result["status"] in _success_statuses:
                conn.execute(
                    "UPDATE jobs SET tailored_resume_path=?, tailored_at=?, keyword_match_rate=?, "
                    "tailor_attempts=COALESCE(tailor_attempts,0)+1 WHERE url=?",
                    (result["path"], now, result.get("keyword_match_rate"), result["url"]),
                )
            else:
                conn.execute(
