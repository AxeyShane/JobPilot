...
def set_proxy(proxy_str: str | None):
    ...
    if proxy_str:
        from jobpilot.discovery.jobspy import parse_proxy
...
def _load_base_urls() -> dict[str, str | None]:
    ...
    from jobpilot.config import load_base_urls
...
def extract_main_content(page) -> str:
    """Extract the main content area as clean markdown via Crawl4AI's
    boilerplate-pruning filter -- shorter and cleaner for the LLM than
    raw HTML, and cheaper on tokens."""
    try:
        html = page.content()
    except Exception:
        return ""
    return html_to_fit_markdown(html, base_url=page.url)[:50000]
...
def extract_with_llm(page, url: str) -> dict:
    ...
    try:
        ...
        # NuExtract-family models take a bare JSON schema via a template
        # kwarg and the raw text as the message -- no instructions. Any
        # other model (the natural-language DETAIL_EXTRACT_PROMPT path)
        # gets the original instructional prompt unchanged.
        if "nuextract" in client.model.lower():
            raw = client.ask(
                content[:30000],
                temperature=0.0,
                max_tokens=2048,
        ...
        log.info("LLM: %d chars in, %.1fs", prompt_len, elapsed)

        from jobpilot.discovery.smartextract import extract_json
...
def _run_detail_scraper(
    ...
    params: list = []
    if only_urls:
        # Fast lane: enrich just the URLs this poll discovered, not the backlog.
        placeholders = ",".join("?" for _ in only_urls)
        where += f" AND url IN ({placeholders})"
        params.extend(only_urls)
    # Newest first within each site batch. The scraper is frequently cut short
...
def run_enrichment(limit: int = 100, workers: int = 1,
                   only_urls: list[str] | None = None) -> dict:
    ...
    Args:
        ...
        only_urls: Restrict enrichment to these job URLs (fast lane).
