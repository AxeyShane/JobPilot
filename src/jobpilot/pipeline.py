...
Usage (via CLI):
    jobpilot run                        # all stages, sequential
    jobpilot run --stream               # all stages, concurrent
    jobpilot run discover enrich        # specific stages
    jobpilot run score tailor cover     # LLM-only stages
...
def _run_tailor(min_score: int = 6, validation_mode: str = "normal", workers: int = 1) -> dict:
...
def _run_cover(min_score: int = 6, validation_mode: str = "normal", workers: int = 1) -> dict:
...
def _run_pdf() -> dict:
    ...
    try:
        from jobpilot.scoring.pdf import batch_convert
        batch_convert()
        return {"status": "ok"}
    except Exception as e:
...
def run_pipeline(
    ...
    console.print(Panel.fit(
        f"[bold]JobPilot Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
