...
from jobpilot.config import CONFIG_DIR
from jobpilot.database import get_connection, init_db, store_jobs, get_stats
...
# Hard cap on how long a single site's collect_page_intelligence() may run.
# page.goto() itself has a 60s timeout, but Response.text() inside the
# on_response handler has no timeout of its own and can hang forever on a
# stalled/streaming response body -- observed in production.
...
def _slugify(text: str) -> str:
    """lowercase, hyphen-joined slug -- e.g. Bayt/NaukriGulf/Foundit URL scheme."""
...
def collect_page_intelligence(url: str, headless: bool = True) -> dict:
    ...
    # Boilerplate-stripped text (Crawl4AI) -- used for strategy-briefing
    # context and bot-block detection. Kept separate from full_html, which
    # Phase 2's CSS-selector generation still needs in structural form.
    intel["fit_markdown"] = html_to_fit_markdown(intel["full_html"], base_url=url)
...
def format_strategy_briefing(intel: dict) -> str:
    ...
    fit_markdown = intel.get("fit_markdown", "")
    if fit_markdown:
        sections.append(f"\nPAGE TEXT (boilerplate stripped, for context):\n{fit_markdown[:2000]}")
...
def _run_one_site(name: str, url: str) -> dict:
    ...
    # Checked against fit_markdown (boilerplate-stripped text) rather than raw
    # HTML -- avoids false positives from the signal words appearing in
    # scripts, class names, or hidden attributes rather than visible copy.
    _block_text = intel.get("fit_markdown", "") or full_html
    _is_captcha = any(s in _block_text.lower() for s in _captcha_signals) if _block_text else False
    ...
    for site in sites:
        ...
        if site_type == "search" and queries:
            for query in queries:
                ...
                expanded_url = expanded_url.replace("{query_slug}", _slugify(query))
...
def _run_all(
    ...
    def _timeout_result(name: str) -> dict:
        return {"status": "FAIL", "name": name, "error": "timeout",
                "total": 0, "titles": 0, "strategy": "timeout"}

    if workers > 1 and len(targets) > 1:
        # Parallel mode. Each future gets its OWN bounded wait -- a single
        # stuck site (observed: Playwright response.text() hanging forever
        # on a stalled/streaming response body, no timeout of its own) no
        # longer blocks the whole run. The stuck worker thread keeps running
        # in the background; ThreadPoolExecutor still feeds queued targets
        # to the other workers as they free up.
        with ThreadPoolExecutor(max_workers=min(workers, len(targets))) as pool:
            ...
            for future in future_to_target:
                ...
                try:
                    r = future.result(timeout=SITE_TIMEOUT_SECONDS)
                except FuturesTimeoutError:
                    log.warning("%s: timed out after %ds -- skipping",
                                target["name"], SITE_TIMEOUT_SECONDS)
                    r = _timeout_result(target["name"])
                results.append(r)
                _process_result(r, target)
    else:
        # Sequential mode (default). Same per-site timeout via a throwaway
        # single-worker pool so one hung site can't wedge the whole run.
        for i, target in enumerate(targets):
            ...
            with ThreadPoolExecutor(max_workers=1) as one_pool:
                fut = one_pool.submit(_run_one_site, target["name"], target["url"])
                try:
                    r = fut.result(timeout=SITE_TIMEOUT_SECONDS)
                except FuturesTimeoutError:
                    log.warning("%s: timed out after %ds -- skipping",
                                target["name"], SITE_TIMEOUT_SECONDS)
                    r = _timeout_result(target["name"])
