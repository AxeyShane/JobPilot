...
def store_jobspy_results(conn: sqlite3.Connection, df, source_label: str) -> tuple[int, int]:
    ...
    for _, row in df.iterrows():
        ...
        # Extract apply URL if JobSpy provided it (guard: pandas gives NaN or None, never "")
        jd = row.get("job_url_direct")
        apply_url = None
        if jd is not None:
            jd_s = str(jd).strip()
            if jd_s and jd_s.lower() != "nan":
                apply_url = jd_s
...
def run_discovery(cfg: dict | None = None) -> dict:
    ...
    if not cfg:
        log.warning("No search configuration found. Run `jobpilot init` to create one.")
