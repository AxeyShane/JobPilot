...
def load_employers() -> dict:
    ...
    Entries with `enabled: false` are skipped -- e.g. the shipped registry
    tags finance/pension employers this way by default, since crawling ~20
    employers irrelevant to the user's target industry just to filter their
    results out afterward is the single biggest avoidable chunk of discover
    time. Users can flip individual entries back on in their own copy.
    """
